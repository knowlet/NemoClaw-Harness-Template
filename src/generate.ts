...
export async function scaffold(destination, { name = 'my-harness', model = 'managed-model' } = {}) {
  ...
  try {
    ...
    pkg.scripts = { demo: 'node bin/nha.mjs demo', validate: 'node bin/nha.js validate adapter.json', doctor: 'node bin/nha.js doctor', test: 'node --test test/*.test.js' };
    ...
    await cp(path.join(root, 'examples/echo/agent.js'), path.join(temp, 'agent.js'));
    ...
    await writeFile(path.join(temp, 'test/harness.test.js'), `// UNOFFICIAL local contract test. Replace the command and cases when adapting a real harness.
...
test('harness contract', async () => {
  ...
  adapter.runtime.command = [process.execPath, fileURLToPath(new URL('../agent.js', import.meta.url))];
