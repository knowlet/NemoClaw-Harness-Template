...
export async function scaffold(destination, { name = 'my-harness', model = 'managed-model' } = {}) {
  ...
  try {
    await cp(path.join(root, 'dist', 'src'), path.join(temp, 'src'), { recursive: true });
    await cp(path.join(root, 'dist', 'bin'), path.join(temp, 'bin'), { recursive: true });
    await cp(path.join(root, 'examples', 'echo'), path.join(temp, 'examples', 'echo'), { recursive: true });
    ...
    pkg.bin = { nha: 'bin/nha.js' };
    pkg.exports = { '.': { types: './src/index.d.ts', import: './src/index.js' }, './testing': { types: './src/testing.d.ts', import: './src/testing.js' } };
    pkg.scripts = { demo: 'node bin/nha.js demo', validate: 'node bin/nha.js validate adapter.json', doctor: 'node bin/nha.js doctor', test: 'node --test test/*.test.mjs' };
